...
def chat(request: Request, chat_request: ChatRequest):
    ...
    try:
        ...
        if not result["success"]:
            api_logger.error(f"Query failed | question: {question} | sql: {sql_query} | duration: {duration:.2f}s | error: {result['error']}")
            return ChatResponse(message="I couldn't retrieve that data. Could you try rephrasing your question?", sql_query=sql_query)
